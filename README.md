...
- **Gitee 单个附件上限 100MB**（GVP 项目 200MB）。超过 200MB 会在上传前直接报错，
  100MB～200MB 之间会告警提示只有 GVP 仓库能传成功。注意上传是跨境的慢链路，
  实测只有 20–90 KB/s（GitHub 托管 runner → Gitee/GitCode），按这个速度
  100MB 的附件基本传不完。
- **附件上传的超时是 30 分钟、最多尝试 2 次；普通 API 调用是 60 秒。**
  之所以要放宽，是因为 Node 内置 fetch（undici）默认 300 秒的 `headersTimeout`
  是**从请求发出就开始计时**的，不等 body 发完——34MB 的产物会稳定在 301 秒被掐断。
  本 action 通过全局 dispatcher 放宽该上限，真正的超时由每个请求自己的 `AbortSignal` 控制。
