...
- **大附件跨境上传必定失败。** Node 内置 fetch（undici）的 `headersTimeout` 默认 300 秒，
  ...
  被掐断（Gitee 上实测只有 20–90 KB/s）。现在通过全局 dispatcher 把它放宽到 61 分钟，
  并由每个请求自己的 `AbortSignal` 做真正的超时控制：普通 API 调用 60 秒，
  上传附件 60 分钟、最多 5 次尝试。注意最坏情况是单个附件耗满 5 小时，而调用方的矩阵
  job 吃默认的 6 小时上限，余量不多。超时后报错会写明等了多久，而不是只抛一个 `TimeoutError`。
